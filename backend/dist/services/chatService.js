...
const calendlyService_1 = require("./calendlyService");
const prisma = new client_1.PrismaClient();
class ChatService {
    // Handle incoming message from any platform
    static async handleIncomingMessage(platform, socialId, text) {
        ...
        const aiResponse = await aiService_1.AIService.generateChatResponse(historyText, text);
        let finalResponseText = aiResponse.text;
        // 4. Handle Actions (Calendly)
        if (aiResponse.action === 'CHECK_CALENDLY') {
            const slots = await calendlyService_1.CalendlyService.checkAvailability();
            if (slots.length > 0) {
    ...
    static async sendReplyToPlatform(platform, recipientId, text) {
